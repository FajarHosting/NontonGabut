...
const ALLOWED_TYPES = new Set(["anime", "donghua", "drakor", "dracin", "movie", "tv"]);
...
router.get("/contents", async (_req, res) => {
  ...
  res.json({
    items: items.map((x) => ({
      ...
      dramaboxBookId: x.dramaboxBookId || ""
    }))
  });
});
...
router.post("/content", async (req, res) => {
  ...
  if (!ALLOWED_TYPES.has(type)) return res.status(400).json({ error: "BAD_TYPE" });
...
// set bookId untuk Dramabox (Vynaa partner API)
router.post("/content/set-dramabox", async (req, res) => {
  const contentId = String(req.body.contentId || "").trim();
  const bookId = String(req.body.bookId || "").trim();
  if (!contentId) return res.status(400).json({ error: "BAD_INPUT" });

  const item = await Content.findById(contentId);
  if (!item) return res.status(404).json({ error: "NOT_FOUND" });

  item.dramaboxBookId = bookId;
  await item.save();
  return res.json({ ok: true });
});

// Sync daftar episode dari Dramabox partner API (hanya metadata episode; video tetap diisi admin)
router.post("/content/sync-dramabox", async (req, res) => {
  const contentId = String(req.body.contentId || "").trim();
  const bookIdInput = String(req.body.bookId || "").trim();
  if (!contentId) return res.status(400).json({ error: "BAD_INPUT" });

  const item = await Content.findById(contentId);
  if (!item) return res.status(404).json({ error: "NOT_FOUND" });

  const bookId = bookIdInput || item.dramaboxBookId;
  if (!bookId) return res.status(400).json({ error: "BOOK_ID_REQUIRED" });

  const base = String(process.env.VYNAA_BASE_URL || "https://vynaa.web.id").replace(/\/+$/, "");
  const apiKey = String(process.env.VYNAA_API_KEY || "").trim();
  if (!apiKey) return res.status(500).json({ error: "MISSING_VYNAA_API_KEY" });
  if (typeof fetch !== "function") return res.status(500).json({ error: "FETCH_NOT_AVAILABLE" });

  const url = `${base}/entertainment/dramabox/dramabox/allepisode?apikey=${encodeURIComponent(apiKey)}&bookId=${encodeURIComponent(bookId)}`;
  ...
  try {
    ...
    if (!r.ok) {
      return res.status(502).json({ error: "PROVIDER_ERROR", status: r.status, body: remote || null });
    }
  } catch (e) {
    return res.status(502).json({ error: "PROVIDER_FETCH_FAILED", message: String(e && e.message ? e.message : e) });
  }

  if (!remote || remote.status !== true || !Array.isArray(remote.data)) {
    return res.status(502).json({ error: "BAD_PROVIDER_RESPONSE" });
  }

  // simpan bookId
  item.dramaboxBookId = bookId;

  const existingByEp = new Map((item.episodes || []).map((e) => [Number(e.episodeNumber), e]));
  let created = 0;
  let updated = 0;

  for (let i = 0; i < remote.data.length; i++) {
    const x = remote.data[i] || {};
    const episodeNumber = Number.isFinite(Number(x.chapterIndex)) ? Number(x.chapterIndex) + 1 : i + 1;
    const chapterName = String(x.chapterName || `EP ${episodeNumber}`).trim();
    const chapterId = String(x.chapterId || "").trim();

    const cur = existingByEp.get(episodeNumber);
    if (!cur) {
      item.episodes.push({
        episodeNumber,
        title: chapterName,
        chapterId,
        thumbUrl: "",
        // default kosong; admin nanti isi videoProvider + vimeoId/url
        videoProvider: "url",
        videoUrl: "",
        vimeoId: ""
      });
      ...
      // update metadata doang, jangan timpa link video yang sudah diisi admin
      const beforeTitle = String(cur.title || "");
      const beforeChapterId = String(cur.chapterId || "");
      if (chapterName && chapterName !== beforeTitle) cur.title = chapterName;
      if (chapterId && chapterId !== beforeChapterId) cur.chapterId = chapterId;
      updated++;
    }
  }
  ...
  return res.json({ ok: true, bookId, created, updated, total: (item.episodes || []).length });
});
